...
export const categoryService: CategoryService = {
  ...
  getById: async (id) =>
    (await categoryModel.getById(id)) ?? "CATEGORY_NOT_FOUND",

  create: async (newCategoryData) => {
    try {
      return (
        (await categoryModel.create(newCategoryData)) ?? "NO_CATEGORY_CREATED"
      );
    } catch (error: unknown) {
  ...
  update: async (id, updateCategoryData) => {
    try {
      return (
        (await categoryModel.update(id, updateCategoryData)) ??
        "CATEGORY_NOT_FOUND"
      );
    } catch (error: unknown) {
  ...
  remove: async (id) =>
    (await categoryModel.delete(id)) ?? "CATEGORY_NOT_FOUND",
};
