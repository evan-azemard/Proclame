...
export const proclamationService: ProclamationService = {
	...
	getById: async (id) => (await proclamationModel.getById(id)) ?? "PROCLAMATION_NOT_FOUND",

	create: async (newProclamationData) => {
		try {
			return (
				(await proclamationModel.create(newProclamationData)) ?? "NO_PROCLAMATION_CREATED"
			);
		} catch (error: unknown) {
	...
	update: async (id, updateProclamationData) => {
		try {
			return (
				(await proclamationModel.update(id, updateProclamationData)) ?? "PROCLAMATION_NOT_FOUND"
			);
		} catch (error: unknown) {
	...
	remove: async (id) => (await proclamationModel.delete(id)) ?? "PROCLAMATION_NOT_FOUND",
};
